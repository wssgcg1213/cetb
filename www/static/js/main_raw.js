...
(function (e) {
    ...
    function c() {
        if (!a) {
            a = e('<div class="box-overlay"></div>')
        }
        e(".dialog-cont").append(a)
    }
    function u() {
        if (!t) {
            t = e('<div class="dialog">' + '<div class="dialog-content"></div>' + "</div>")
        }
        i = e(".dialog-content", t);
        e(".dialog-cont").append(t);
        t.fadeIn(300)
    }
    function d() {
        t.fadeIn();
    ...
    var p = {
        ...
        success: function (t) {
            var a = t || "操作成功!";
            this.open("<p class='dialog-success'>" + a + "</p>");
            setTimeout(function () {
                e.Dialog.close()
            }, parseInt(arguments[1]) || 2e3)
        },
        fail: function (t) {
            var a = t || "操作失败!";
            this.open("<p class='dialog-fail'>" + a + "</p>");
            setTimeout(function () {
                e.Dialog.close()
            }, parseInt(arguments[1]) || 2e3)
        },
...
(function (e, t) {
    ...
    var o = new t({
        ...
        methods: {
            query: function (t) {
                ...
                var a = u();
                if ("152" === a) {
                    e.Dialog.fail("2015年12月的考试信息还未公布, 请关注公布时间!");
                    return false
                } else if ("id" === a) {
                    e.Dialog.fail("请输入正确的准考证号!");
                    return false
                } else if ("name" === a) {
                    e.Dialog.fail("请输入正确的姓名!");
                    return false
                }
                e.Dialog.loading();
                ...
                e.post("/api/v2/basic", {
                    ...
                    clearTimeout(i);
                    if (t && t.status == 0) {
                        var tid = FUCK(t.data.ticket, t.info);
                        e.Dialog.success("查询成功, 载入中...");
                        e(".tabs-container, .pager").hide();
                        var g = JSON.parse(FUCK(t.data.grade, t.info));
                        setTimeout(function () {
                            l.tname = t.data.name;
                            l.tschool = t.data.school;
                            l.all = g.all;
                            l.writing = g.writing;
                            l.reading = g.reading;
                            l.listening = g.listening;
                            l.tid = tid;
                            l.ttype = c(tid);
                            o.show = false;
                            setTimeout(function () {
    ...
    var r = new t({
        ...
        methods: {
            query: function (t) {
                t.preventDefault();
                if (!this.username) {
                    return e.Dialog.fail("请填写姓名!")
                }
                if (!this.province) {
                    return e.Dialog.fail("请选择学校省份!")
                }
                if (!this.school) {
                    return e.Dialog.fail("请选择学校!")
                }
                if (!this.type) {
                    return e.Dialog.fail("请选择四/六级!")
                }
                e.post("/api/v2/noticket", {
                    name: this.username,
                    school: this.school,
                    cetType: this.type
                }).success(function (t) {
    ...
    r.$watch("province", function (e) {
        r["school"] = "";
        var t;
        try {
            t = i[e][1].map(function (e) {
                return {
                    text: e,
                    value: e
                }
    ...
    function c(e) {
        ...
        } else if (e.length == 14) return "英语口语";
        return ""
    }
    function u() {
        var e = o.username,
        ...
        if (t.length == 15 && t.substr(6, 3) != "151" || c(t) == "") {
            return "id"
        }
        if (t.length == 14 && t.substr(0, 3) != "151" || c(t) == "") {
            return "id"
        }
        return true
    }
    var d = new Swiper("#tab-container", {
        speed: 500,
        onSlideChangeStart: function () {
            e(".tab .selected").removeClass("selected");
            e(".tab li").eq(d.activeIndex).addClass("selected")
        }
    });
    e(".tab li").on("touchstart mousedown", function (t) {
        t.preventDefault();
        e(".tab .selected").removeClass("selected");
        e(this).addClass("selected");
        d.swipeTo(e(this).index())
    });
    e(".tab li").click(function (e) {
        e.preventDefault()
    });
